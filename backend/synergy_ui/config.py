...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(SYNERGY_UI_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = SYNERGY_UI_DIR / "migrations"
...
class AppConfig:
    ...
    def __init__(
        ...
        redis_key_prefix: str = "synergy-ui",
    ):
        if redis_url:
...
if JWT_EXPIRES_IN.value == "-1":
    log.warning(
        "âš ï¸  SECURITY WARNING: JWT_EXPIRES_IN is set to '-1'\n"
        "    See: https://docs.synergyui.com/getting-started/env-configuration\n"
    )
...
def load_oauth_providers():
    ...
    if configured_providers and not OPENID_PROVIDER_URL.value:
        ...
        log.warning(
            f"âš ï¸  OAuth providers configured ({provider_list}) but OPENID_PROVIDER_URL not set - logout will not work!"
        )
        log.warning(
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", SYNERGY_UI_DIR / "static")).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.synergyui.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.synergyui.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.synergyui.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Synergy UI + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.synergy-ui.svc.cluster.local:11434"
...
- { "title": "ðŸª Perfect Chocolate Chip Recipe" },
...
MILVUS_COLLECTION_PREFIX = os.environ.get("MILVUS_COLLECTION_PREFIX", "SYNERGY_UI")
...
QDRANT_COLLECTION_PREFIX = os.environ.get("QDRANT_COLLECTION_PREFIX", "synergy-ui")
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "SYNERGY_UI_collections"
)
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "synergy-ui-index")
